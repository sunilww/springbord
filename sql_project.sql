...
the member name. */
SELECT DISTINCT 
CONCAT(mems.surname, " ",mems.firstname) AS member_name,
...
SELECT facs.name as facility_name, 
	CASE 
	WHEN  mems.surname = 'GUEST'
	THEN  mems.surname
	ELSE  CONCAT(mems.surname, ", ", mems.firstname)
	END AS member_name,
	CASE 
	WHEN  mems.surname = 'GUEST'
	THEN  facs.guestcost * bks.slots
...
WHERE bks.starttime BETWEEN '2012-09-14' AND '2012-09-15'
HAVING cost > 30
ORDER BY cost DESC
...
SELECT facs.name as facility_name, 
	...
	ELSE  facs.membercost * bks.slots
	END AS cost
FROM (SELECT * from `Bookings` where starttime BETWEEN '2012-09-14' AND '2012-09-15')bks
INNER JOIN `Facilities` facs
ON bks.facid = facs.facid
INNER JOIN `Members` mems
ON bks.memid = mems.memid
